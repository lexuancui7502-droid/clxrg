...
from metrics.natural_language import NaturalLanguage
from metrics.Radgraph import F1RadGraph
from metrics.f1chexbert import F1CheXbert
# from save_path import enumerated_save_path
import os
import pandas as pd
import time
import torch
...
# # 继承自 NaturalLanguage，用于CheXbert相关的评测
...
#             save_path = "/media/yanxiuying/DynRefer/z-process/CE_scores.csv"
#             class_scores_dict = {
...
class RadGraphMetrics(NaturalLanguage):
    ...
    def __init__(
        ...
        self.mbatch_size = mbatch_size
        print("mbatch_size:",self.mbatch_size)
        # self.save_class_scores = save_class_scores
        # self.save_outputs = save_outputs
        # self.exp_dir = exp_dir

        # ✅ 加载模型（只加载一次）
        from chexbert import F1RadGraph  # 确保不循环导入
        self.f1radgraph_partial = F1RadGraph(
            reward_level='partial',
            model_path=self.radgraph_path
        ).to(self.device)

        self.pairs = []  # 外部调用时赋值 [(pred, ref, id), ...]
    ...
    def compute(self):
        if not self.pairs:
            raise ValueError("pairs 为空，请先赋值 self.pairs = [(pred, ref, id), ...]")
        ...
        #    
        # ✅ 使用已加载的模型计算分数
        partial_mean_reward, reward, hypothesis_ann, reference_ann = self.f1radgraph_partial(
            hyps=table['hyps'],
            refs=table['refs']
        )
...
class F1CheXbertMetrics(NaturalLanguage):
    ...
    def __init__(
        ...
        # # ✅ 将模型加载到初始化中
        # from chexbert import F1CheXbert  # 确保不会循环导入
        # self.f1chexbert = F1CheXbert(
        #     chexbert_checkpoint=self.chexbert_path,
        #     model_checkpoint=self.model_path,
        #     tokenizer_checkpoint=self.model_path
        # ).to(self.device)

        self.pairs = []  # ✅ 显式声明 pairs 属性
    ...
    def compute(self):
        ...
        # accuracy, accuracy_per_sample, chexbert_all, chexbert_5 = self.f1chexbert(
        #     hyps=table['hyps'],
        #     refs=table['refs']
        # )
