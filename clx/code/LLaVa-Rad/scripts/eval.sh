...
MODEL_PATH="${MODEL_PATH:-$WEIGHTS_ROOT/llava-rad}"                # LLaVA-Rad（本地）
QUERY_FILE="${QUERY_FILE:-$BASE_ROOT/data/llava-rad-mimic-cxr-annotations-1.0.0/_mini_100.json}"
IMAGE_FOLDER="${IMAGE_FOLDER:-/media/datasets/physionet.org/files/mimic-cxr-jpg/2.0.0/files}"

STAMP="$(date +%m%d_%H%M)"
OUTDIR="$ROOT_DIR/results/llavarad_${STAMP}"
RUN_NAME="llavarad_${STAMP}"
