    ...
    with gr.Blocks(title="LLaVA", theme=gr.themes.Default(), css=block_css) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                mimic_cxr_dir = "/mnt/hanoverdev/gpt-rad/data/mimic-cxr/"
                with open(os.path.join(mimic_cxr_dir, "llava_json/chat_dev_MIMIC_CXR_frontal_gpt4extract_v2.json")) as f:
                    examples = [
                        [
                            os.path.join(mimic_cxr_dir, "mimic-cxr-jpg/files", ex["image"]),
                            "\n".join([c["value"].replace("<image>", "").strip() for c in ex["conversations"]])
                        ]
                        for ex in json.load(f)[:5]
                    ]
