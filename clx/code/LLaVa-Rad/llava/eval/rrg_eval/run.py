...
# [2025-10-16] 稳健化导入：优先相对“子包”导入；失败再绝对导入；再失败回退把子包路径塞到 sys.path。
try:
    # 优先：相对导入到里层子包
    from .rrg_eval import chexbert as _chexbert
    from .rrg_eval import rouge as _rouge
    from .rrg_eval import f1radgraph as _f1radgraph
    from .rrg_eval.f1radgraph import F1RadGraphv2
    from .rrg_eval.factuality_utils import CONDITIONS
except Exception:
    try:
        # 其次：绝对导入（从仓库根作为包）
        from llava.eval.rrg_eval.rrg_eval import chexbert as _chexbert
        from llava.eval.rrg_eval.rrg_eval import rouge as _rouge
        from llava.eval.rrg_eval.rrg_eval import f1radgraph as _f1radgraph
        ...
        # 兜底：手动把里层子包加入 sys.path 再导
        import os, sys
        _PKG_DIR = os.path.dirname(__file__)
        _INNER = os.path.join(_PKG_DIR, "rrg_eval")
        if _INNER not in sys.path:
            sys.path.insert(0, _INNER)
        import chexbert as _chexbert
        import rouge as _rouge
        import f1radgraph as _f1radgraph
...
# 2024.10.16 把原来的 bleu1 改成了下面这个版本，增加了一个 fallback 机制
...
def bleu1(predictions, references, bootstrap_ci=False):
    """
    Compute corpus BLEU-1 with a safe fallback.
    Try `evaluate.load("bleu")` first (with max_order=1).
    If it fails (e.g., module fetch issue), fall back to a tiny internal BLEU-1.
    """
    try:
        import evaluate
        try:
        ...
        print("[WARN] evaluate not available, using internal BLEU-1:", e)

    # -------- Internal corpus BLEU-1 (with brevity penalty) --------
    ...
    for hyp, ref in zip(predictions, references):
        ...
        c_len += len(hyp_toks)
        r_len += len(ref_toks)
        match += sum((Counter(hyp_toks) & Counter(ref_toks)).values())

    if c_len == 0:
        return 0.0
    p1 = match / c_len
    bp = math.exp(1 - r_len / c_len) if c_len < r_len and c_len > 0 else 1.0
    return bp * p1
...
def rougel(predictions, references, bootstrap_ci: bool = False):
    if bootstrap_ci:
        ...
        # [2024.10.16] 修改了别用名称 rrg_eval，改成了 _rouge
...
def radgraph(predictions, references, bootstrap_ci: bool = False):
    if bootstrap_ci:
        ...
        # [2024.10.16] 修改了别用名称 rrg_eval，改成了 _f1radgraph
...
def chexbert(predictions, references, bootstrap_ci: bool = False):
    ...
    # [2024.10.16] 修改了别用名称 rrg_eval，改成了 _chexbert
...
def main(
    ...
    if wandb:
        ...
        # 2024.10.16 将 metric 和 index 都转成字符串；并把 numpy 标量转成 Python 标量
        for metric in main_results.columns:
            for idx in main_results.index:
                k = str(metric)                      # 统一字符串化列名
                k = f"{k}-{str(idx)}"                # 统一追加行索引（即使是 0/整数也转成字符串）
                val = main_results.loc[idx, metric]  # 取值

                # 将 numpy 标量转换为 Python float/int，避免 JSON/CSV 警告
                try:
                    import numpy as _np
