...
#     test
# ) 
# [2025-10-16] 稳健导入（保持旧名字，不改后续代码）：
# 1) 优先相对导入（从里层 rrg_eval 子包）
try:
    from .factuality_utils import (
        CheXbert_CONDITIONS, CONDITIONS_5, map_to_binary, CONDITIONS
    )
    from .factuality_eval import (
        generate_classification_report,
        # 如还有其它用到的函数，也一起列出来
        test
    )
# 2) 兜底：从仓库根的绝对路径导入（同样保留相同的名字）
except Exception:
    from llava.eval.rrg_eval.rrg_eval.factuality_utils import (
        CheXbert_CONDITIONS, CONDITIONS_5, map_to_binary, CONDITIONS
    )
    from llava.eval.rrg_eval.rrg_eval.factuality_eval import (
        generate_classification_report,
        # 与上面对齐
        test
    )
